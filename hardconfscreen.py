...
confTemp = singletonconfigurador.stateConfigure.get_instance().listDif
...
def game_configuration():
    ...
    while intro == True:
        ...
        for event in pygame.event.get():
            ...
            textLinea.append("DIFICULTAD: DIFICIL")
            ...
            if((0.5*display_ancho+0.2*display_ancho) > mouse[0] and 0.5*display_ancho < mouse[0]) and (0.8*display_altura+50 > mouse[1] and 0.8*display_altura< mouse[1]):
                if click[0] == 1:
                    transfercreator.transferirParams(confTemp, conf, "dificil")
                
            dManager.boton("Regresar", params, None, mediumconfscreen.game_configuration)
            dManager.boton("Regresar al menu", params2, None, oriongame.game_intro)
